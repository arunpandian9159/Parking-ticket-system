...
export default function ParkingRates() {
    ...
    if (loading) return <div className="p-8">Loading rates...</div>

    return (
        <div className="max-w-4xl mx-auto space-y-8">
            <div>
                <h1 className="text-3xl font-bold text-gray-900">Parking Rates Configuration</h1>
                <p className="text-gray-500">Set hourly rates for different vehicle types.</p>
            </div>
                ...
                <div className="grid gap-4">
                    {rates.map((rate) => (
                        <div key={rate.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                            <div className="w-1/3">
                                <span className="font-medium text-gray-900">{rate.vehicle_type}</span>
                            </div>
                    ...
                    {rates.length === 0 && (
                        <p className="text-gray-500 text-center py-4">No rates configured yet.</p>
                    )}
                </div>
            </div>

            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h2 className="text-lg font-semibold mb-4">Add New Vehicle Type</h2>
