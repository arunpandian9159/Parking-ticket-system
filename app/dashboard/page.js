...
import { getProfile, getUserDisplayInfo } from '@/lib/profile'
      ...
      >
        <div className="p-4 sm:p-6 lg:p-8 flex flex-col h-full relative z-10">
          ...
          <div className="relative mb-4 sm:mb-6">
            <div
              className={`absolute inset-0 bg-linear-to-br ${styles.gradient} rounded-2xl blur-xl opacity-0 group-hover:opacity-50 transition-opacity duration-500`}
            />
            <div
              className={`relative w-14 h-14 sm:w-16 sm:h-16 lg:w-20 lg:h-20 ${styles.bg} ${styles.bgHover} rounded-xl sm:rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-110`}
            >
          ...
          <ul className="space-y-1.5 sm:space-y-2 mb-4 sm:mb-6">
            {features.map((feature, index) => (
              ...
              >
                <div
                  className={`w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-linear-to-r ${styles.gradient}`}
                />
                {feature}
              </li>
            ))}
          ...
          {/* CTA */}
          <div
            className={`${styles.text} font-semibold text-sm sm:text-base flex items-center gap-2 group-hover:gap-3 transition-all duration-300`}
          >
            Enter {title.split(' ')[0]}
...
function QuickStatCard({ icon: Icon, label, value, suffix = '', trend, trendUp = true, color }) {
  ...
  return (
    <div
      className={`bg-linear-to-br ${colorClasses[color]} border rounded-xl p-3 sm:p-4 lg:p-5 hover:scale-[1.02] transition-transform duration-300`}
    >
...
function ActivityItem({ icon: Icon, title, time, color }) {
  ...
  return (
    <div className="flex items-center gap-3 sm:gap-4 p-2 sm:p-3 rounded-lg hover:bg-secondary/50 transition-colors group cursor-pointer">
      <div
        className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl ${bgColors[color]} flex items-center justify-center group-hover:scale-110 transition-transform`}
      >
...
function QuickAction({ icon: Icon, label, href, color = 'teal' }) {
  ...
  return (
    <Link href={href}>
      <button
        className={`w-full flex items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-xl border border-border bg-card/50 text-muted-foreground ${colorClasses[color]} transition-all duration-300 group`}
      >
...
function SettingsModal({ isOpen, onClose, user, profile, userInfo }) {
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Moon
                          className={`w-4 h-4 ${
                            theme === 'dark' ? 'text-teal-500' : 'text-muted-foreground'
                          }`}
                        />
                        <span
                          className={`text-sm font-medium ${theme === 'dark' && 'text-teal-500'}`}
                        >
                          Dark Mode
                        </span>
                      </div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Sun
                          className={`w-4 h-4 ${
                            theme === 'light' ? 'text-teal-500' : 'text-muted-foreground'
                          }`}
                        />
                        <span
                          className={`text-sm font-medium ${theme === 'light' && 'text-teal-500'}`}
                        >
                          Light Mode
                        </span>
                      </div>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    },
                    {
                      ...
                      desc: 'Get a weekly report of your activity',
                      icon: Calendar,
                    },
                  ].map(item => (
                    <div
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`w-12 h-6 rounded-full transition-colors duration-200 relative ${
                          notifications[item.id] ? 'bg-teal-500' : 'bg-secondary'
                        }`}
                      >
                        <div
                ...
                <div className="p-4 rounded-xl border border-border bg-card space-y-4">
                  <div className="flex items-center gap-4">
                    <Avatar
                      ...
                      initials={userInfo?.initials || user?.email?.[0].toUpperCase()}
                      size="xl"
                    />
                    <div>
                      <p className="font-semibold text-lg">
                        {userInfo?.name || user?.user_metadata?.full_name || 'User'}
                      </p>
                      <p className="text-muted-foreground text-sm">{user?.email}</p>
                      <div className="mt-2 flex items-center gap-2">
                        <div className="inline-flex items-center px-2 py-1 rounded-full bg-emerald-500/10 text-emerald-500 text-xs font-medium border border-emerald-500/20">
                          <Check className="w-3 h-3 mr-1" />
                          Active Account
                        </div>
                        {userInfo?.provider === 'google' && (
                          <div className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-secondary border border-border">
...
export default function ChoiceDashboard() {
  ...
  const recentActivity = [
    ...
    {
      icon: Activity,
      title: 'Vehicle TN02CD5678 checked out',
      time: '18 minutes ago',
      color: 'amber',
    },
  ]
  ...
  const userInfo = getUserDisplayInfo(user, profile)
            ...
            <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 sm:gap-3 mb-1 sm:mb-2">
                  ...
                  {userInfo.provider === 'google' && (
                    <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-secondary border border-border">
                      <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
                        <path
                          d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                          fill="#4285F4"
          ...
          <FadeIn delay={0.2} className="lg:col-span-2">
            <div className="mb-4 sm:mb-6">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">
                Choose a portal based on your role and tasks
              </p>
            </div>
